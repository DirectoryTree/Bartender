...
$cleaned = DinnerParty::clean($array);
...
$cleaned = DinnerParty::config($config)->clean($input);
```
...
$cleanedContent = DinnerParty::config('comments')->clean(request('content'));
```
...
class Post extends Model
{
    ...
    {
        return DinnerParty::clean($value);
    }
}
```
